...
[示例代码](#示例代码)
[app.base 文档](../../../reference/app/base/init.md)

## 创建设备模型

如应用是某个设备的数据源，即需要向 FreeIOE 发布设备数据，则需要在应用启动时，创建设备模型以及实例。
...
1. name: 设备名称(云平台唯一名称, 如，某某锅炉，某某电表
2. description: 为设备产品的具体描述信息，如某某品牌三相交流智能电表
3. inst: 为选填属性，如 1#锅炉，3#电表， 办公室空调一号等。 是标记设备的友好名称
4. series: 为设备产品的系列号，如 S5500, S7-300
...
类似meta信息，input/output/command除了本文档定义的属性外，也是可以自定义属性，注意事项如同meta信息。
...
如果设备具备可以对外输出的能力，模拟量输出、开关量输出、可调节输出电压等等时，可以在设备模型中声明这些输出项。

而具备输出项的设备，用户就可从而平台可以进行设备数据下置。
